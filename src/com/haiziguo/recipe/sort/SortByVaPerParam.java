...
public class SortByVaPerParam implements Comparator<Object>{
	...
	public int compare(Object o1, Object o2) {
		 Food s1 = (Food) o1;
		 Food s2 = (Food) o2;
		 if(s1.getVa()<Define.FLOAT_ZERO || s2.getVa()<Define.FLOAT_ZERO){
			 	if(order)
		 ...
		 switch(param){
		 	...
		 	case 11:
		 		if(order)
		 			return ((Float)(s1.getVa()/s1.getVe())).compareTo(((Float)(s2.getVa()/s2.getVe())));
		 		else
		 			return ((Float)(s2.getVa()/s2.getVe())).compareTo(((Float)(s1.getVa()/s1.getVe())));
		 	case 12:
		 		if(order)
		 			return ((Float)(s1.getVa()/s1.getNa())).compareTo(((Float)(s2.getVa()/s2.getNa())));
		 		else
		 			return ((Float)(s2.getVa()/s2.getNa())).compareTo(((Float)(s1.getVa()/s1.getNa())));
		 	case 7:
		 	default:
		 		if(order)
		 			return s1.getVa().compareTo(s2.getVa());
